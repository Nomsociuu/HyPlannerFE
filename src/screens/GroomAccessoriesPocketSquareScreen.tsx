...
const GroomAccessoriesPocketSquareScreen = () => {
  const navigation = useNavigation();
  const [menuVisible, setMenuVisible] = useState(false);
  const [items, setItems] = useState<Style[]>([]);
  const { selectedGroomPocketSquare, toggleGroomPocketSquare } = useSelection();

  useEffect(() => {
    const fetch = async () => {
      try {
        const r = await groomSuitService.getVestPockets();
        setItems(r.data || []);
      } catch (e) {}
    };
    fetch();
    ...
    <SafeAreaView style={[styles.container, { paddingTop: topPad }] }>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={styles.headerSubtitle}>Túi áo</Text>
        </View>
      ...
      <GroomAccessoriesMenu visible={menuVisible} onClose={() => setMenuVisible(false)} currentScreen="GroomAccessoriesPocketSquare" />
        ...
        <View style={styles.grid}>
          {items.map(item => (
            <WeddingItemCard
              ...
              name={item.name}
              image={item.image}
              isSelected={selectedGroomPocketSquare.includes(item._id)}
              onSelect={async () => await toggleGroomPocketSquare(item._id)}
            />
          ))}
        </View>

        <TouchableOpacity style={styles.actionButton} onPress={() => navigation.navigate('GroomAccessoriesDecor' as never)}>
          <Text style={styles.actionButtonText}>Chọn trang trí</Text>
...
const styles = StyleSheet.create({
  ...
  actionButton: { backgroundColor: '#F9CBD6', paddingVertical: 8, paddingHorizontal: 16, borderRadius: 100, marginTop: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', alignSelf: 'center' },
  actionButtonText: { color: '#000000', textAlign: 'center', fontSize: 14, fontFamily: fonts.montserratSemiBold, marginRight: 4 },
});

export default GroomAccessoriesPocketSquareScreen;
