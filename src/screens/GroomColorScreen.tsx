...
const GroomColorScreen = () => {
  const navigation = useNavigation();
  const [items, setItems] = useState<Style[]>([]);
  const { selectedGroomVestColors, toggleGroomVestColor } = useSelection();
    ...
    <SafeAreaView style={[styles.container, { paddingTop: topPad }] }>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          ...
          <Text style={styles.headerSubtitle}>Màu sắc</Text>
        </View>
        <View style={{ width: 24 }} />
      </View>
        ...
        <View style={styles.grid}>
          {items.map(item => (
            <WeddingItemCard
              ...
              name={item.name}
              image={item.image}
              isSelected={selectedGroomVestColors.includes(item._id)}
              onSelect={async () => await toggleGroomVestColor(item._id)}
            />
          ))}
        </View>

        <TouchableOpacity style={styles.actionButton} onPress={() => navigation.navigate('GroomAccessoriesLapel' as never)}>
...
export default GroomColorScreen;
