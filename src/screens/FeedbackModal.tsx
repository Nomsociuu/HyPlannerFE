...
import { createFeedback, editFeedback } from "src/service/feedbackService";
import { selectCurrentUser } from "src/store/authSlice";
import { Feedback } from "src/store/feedbackSlice";
...
const FeedbackModal: React.FC<FeedbackDialogProps> = ({
    ...
    const isLoading = useSelector((state: any) => state.feedback.createFeedback.isLoading);
                    ...
                    {showSuccess ? (
                        ...
                        <>
                            ...
                            <View style={styles.starContainer}>
                                {[1, 2, 3, 4, 5].map((s) => (
                                    <TouchableOpacity key={s} onPress={() => setStar(s)} disabled={isLoading}>
                                        <Icon
                                            source={s <= star ? "star" : "star-outline"}
                                            size={32}
                                            color={s <= star ? COLORS.starActive : COLORS.starInactive}
                                        />
                                    </TouchableOpacity>
                                ))}
                            </View>
