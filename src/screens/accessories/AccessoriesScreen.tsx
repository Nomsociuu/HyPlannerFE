...
const AccessoriesScreen = () => {
  ...
  useEffect(() => {
    const fetchVeils = async () => {
      setIsLoading(true);
      try {
        const response = await weddingCostumeService.getAllVeils();
        setVeils(response.data);
        setError(null);
      } catch (err: any) {
  ...
  return (
    <SafeAreaView style={[styles.container, { paddingTop: topPad }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Phụ kiện - Voan</Text>
      </View>
            ...
            {veils.map((item) => (
              <View style={{ width: ITEM_WIDTH }} key={item._id}>
                <WeddingItemCard
                  ...
                  name={item.name}
                  image={item.image}
                  isSelected={selectedVeils.includes(item._id)}
                  onSelect={async () => await toggleVeilSelection(item._id)}
                />
              </View>
            ))}
          </View>
      ...
      <View style={styles.actionButtonContainer}>
        <TouchableOpacity
          ...
          onPress={async () => {
            ...
            if (
              isInWizard &&
              currentStep === AlbumWizardStep.ACCESSORIES_VEIL
            ) {
              nextStep();
            }
            navigation.navigate("AccessoriesJewelry" as never);
          }}
        >
          <Text style={styles.actionButtonText}>Chọn trang sức</Text>
          <ChevronLeft
            size={16}
            color="#000000"
