        ...
        return (
            <View style={styles.rowFront}>
                <List.Item
                    ...
                    description={
                        <>
                            <Text style={{ color: "#7f7878ff", fontSize: responsiveFont(10) }}>Dự kiến: {task.expectedBudget ? task.expectedBudget.toLocaleString() + "đ" : 0 + "đ"} - Thực tế: {task.actualBudget ? task.actualBudget.toLocaleString() + "đ" : 0 + "đ"}</Text>
                        </>
                    }
                    titleStyle={[
                        styles.taskText,
    ...
    const renderStage = ({ item: stage }: { item: any }) => (
        <View style={{ borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 1, marginBottom: 10 }}>
            <List.Accordion
                ...
                description={
                    <>
                        <Text>{`Dự kiến: ${stage.totalExpectedBudget.toLocaleString()}đ`} / {`Thực tế: ${stage.totalActualBudget.toLocaleString()}đ`}</Text>
                    </>
                }
