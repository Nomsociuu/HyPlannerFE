...
export default function BrideAoDaiMaterialScreen() {
  const navigation = useNavigation();
  const [menuVisible, setMenuVisible] = useState(false);
  const [items, setItems] = useState<Material[]>([]);
  const { selectedBrideEngageMaterials, toggleBrideEngageMaterial, saveSelections } = useSelection();
  useEffect(() => { 
    brideEngageService.getAllBrideEngageMaterials()
      ...
      .catch(error => {
        console.log('Materials API Error:', error);
      });
  }, []);
  ...
  return (
    <SafeAreaView style={[styles.container, { paddingTop: topPad }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}><Text style={styles.headerTitle}>Áo dài</Text><Text style={styles.headerSubtitle}>Chất liệu</Text></View>
      ...
      <EngagementBrideMenu visible={menuVisible} currentScreen="BrideAoDaiMaterial" onClose={()=>setMenuVisible(false)} />
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.grid}>
          {items.map(it => (
            <WeddingItemCard key={it._id} id={it._id} name={it.name} image={it.image} isSelected={selectedBrideEngageMaterials.includes(it._id)} onSelect={async () => await toggleBrideEngageMaterial(it._id)} />
          ))}
        </View>
        <TouchableOpacity style={styles.actionButton}         onPress={async () => {
          await saveSelections();
          navigation.navigate('BrideAoDaiPattern' as never);
        }}>
          <Text style={styles.actionButtonText}>Chọn hoa văn & trang trí</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
