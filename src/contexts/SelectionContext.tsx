...
export const SelectionProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Tone color
  const [selectedWeddingToneColors, setSelectedWeddingToneColors] = useState<
    string[]
  >([]);
  const [selectedEngageToneColors, setSelectedEngageToneColors] = useState<
    string[]
  >([]);
  // Groom-specific
  const [selectedGroomLapel, setSelectedGroomLapel] = useState<string[]>([]);
  const [selectedGroomPocketSquare, setSelectedGroomPocketSquare] = useState<
    string[]
  >([]);
  const [selectedGroomDecor, setSelectedGroomDecor] = useState<string[]>([]);
  const [selectedGroomVestStyles, setSelectedGroomVestStyles] = useState<
    string[]
  >([]);
  const [selectedGroomVestMaterials, setSelectedGroomVestMaterials] = useState<
    string[]
  >([]);
  const [selectedGroomVestColors, setSelectedGroomVestColors] = useState<
    string[]
  >([]);

  // Bride-engage specific
  const [selectedBrideEngageStyles, setSelectedBrideEngageStyles] = useState<
    string[]
  >([]);
  const [selectedBrideEngageMaterials, setSelectedBrideEngageMaterials] =
    useState<string[]>([]);
  const [selectedBrideEngagePatterns, setSelectedBrideEngagePatterns] =
    useState<string[]>([]);
  const [selectedBrideEngageHeadwears, setSelectedBrideEngageHeadwears] =
    useState<string[]>([]);

  // Groom-engage specific
  const [selectedGroomEngageOutfits, setSelectedGroomEngageOutfits] = useState<
    string[]
  >([]);
  const [selectedGroomEngageAccessories, setSelectedGroomEngageAccessories] =
    useState<string[]>([]);

  const [hasExistingSelection, setHasExistingSelection] =
  ...
  const saveAllSelections = useCallback(async () => {
    ...
    try {
      ...
      const hasWeddingSelections =
        ...
        selectedVeils.length > 0 ||
        selectedJewelry.length > 0 ||
        selectedHairpins.length > 0 ||
      ...
      const hasToneColorSelections =
        selectedWeddingToneColors.length > 0 ||
        selectedEngageToneColors.length > 0;
        ...
        await userSelectionService.createSelection(
          {
            ...
            brideEngageMaterialIds: Array.from(
              new Set(selectedBrideEngageMaterials)
            ),
            brideEngagePatternIds: Array.from(
              new Set(selectedBrideEngagePatterns)
            ),
            brideEngageHeadwearIds: Array.from(
              new Set(selectedBrideEngageHeadwears)
            ),
          },
          "bride-engage"
        );
      ...
      const hasGroomEngageSelections =
        selectedGroomEngageOutfits.length > 0 ||
        selectedGroomEngageAccessories.length > 0;
      if (dirtyGroomEngageRef.current && hasGroomEngageSelections) {
        await userSelectionService.deleteSelection("groom-engage");
        await userSelectionService.createSelection(
          {
            groomEngageOutfitIds: Array.from(
              new Set(selectedGroomEngageOutfits)
            ),
            groomEngageAccessoryIds: Array.from(
              new Set(selectedGroomEngageAccessories)
            ),
          },
          "groom-engage"
        );
      ...
      setHasExistingSelection(
        hasWeddingSelections ||
          hasVestSelections ||
          hasBrideEngageSelections ||
          hasGroomEngageSelections ||
          hasToneColorSelections
      );
    } catch (error) {
  ...
  const toggleStyleSelection = useCallback(
  ...
  const toggleNecklineSelection = useCallback(
    async (necklineId: string) => {
  ...
  const toggleVeilSelection = useCallback(
    async (veilId: string) => {
  ...
  const toggleJewelrySelection = useCallback(
    async (jewelryId: string) => {
  ...
  const toggleHairpinSelection = useCallback(
    async (hairpinId: string) => {
  ...
  const toggleCrownSelection = useCallback(
    async (crownId: string) => {
  ...
  const toggleFlowerSelection = useCallback(
  ...
  const toggleWeddingToneColor = useCallback(
    async (id: string) => {
      toggleSelection(
  ...
  const toggleEngageToneColor = useCallback(
    async (id: string) => {
      toggleSelection(
  ...
  const toggleGroomLapel = useCallback(
    async (id: string) => {
  ...
  const toggleGroomPocketSquare = useCallback(
    async (id: string) => {
      toggleSelection(
        selectedGroomPocketSquare,
  ...
  const toggleGroomDecor = useCallback(
    async (id: string) => {
  ...
  const toggleGroomVestStyle = useCallback(
    async (id: string) => {
  ...
  const toggleGroomVestMaterial = useCallback(
    async (id: string) => {
      toggleSelection(
        selectedGroomVestMaterials,
  ...
  const toggleGroomVestColor = useCallback(
    async (id: string) => {
  ...
  // Bride-engage toggles
  const toggleBrideEngageStyle = useCallback(
    async (id: string) => {
      toggleSelection(
        selectedBrideEngageStyles,
  ...
  const toggleBrideEngageMaterial = useCallback(
    async (id: string) => {
      toggleSelection(
        selectedBrideEngageMaterials,
  ...
  const toggleBrideEngagePattern = useCallback(
    async (id: string) => {
      toggleSelection(
        selectedBrideEngagePatterns,
  ...
  const toggleBrideEngageHeadwear = useCallback(
    async (id: string) => {
      toggleSelection(
        selectedBrideEngageHeadwears,
  ...
  // Groom-engage toggles
  const toggleGroomEngageOutfit = useCallback(
    async (id: string) => {
      toggleSelection(
        selectedGroomEngageOutfits,
  ...
  const toggleGroomEngageAccessory = useCallback(
    async (id: string) => {
      toggleSelection(
        selectedGroomEngageAccessories,
      ...
      if (
        ...
        try {
          ...
          const isNotFound =
            ...
            message === "No pinned wedding-dress selection found";
      ...
      if (
        ...
        try {
          ...
          const isNotFound =
            status === 404 ||
            message === "No pinned selection found" ||
            message === "No pinned vest selection found";
          if (!isNotFound) throw error;
        }
      ...
      if (
        ...
        try {
          ...
          const isNotFound =
            status === 404 ||
            message === "No pinned selection found" ||
            message === "No pinned bride-engage selection found";
          if (!isNotFound) throw error;
        }
      ...
      if (
        ...
        try {
          ...
          const isNotFound =
            status === 404 ||
            message === "No pinned selection found" ||
            message === "No pinned groom-engage selection found";
          if (!isNotFound) throw error;
        }
    ...
    async (
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "Could not get existing albums, using default count:",
            error
          );
        }
